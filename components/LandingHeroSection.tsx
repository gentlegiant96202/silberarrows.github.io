...
export function LandingHeroSection({ config }: LandingHeroSectionProps) {
      ...
      <div className="hero-content">
        ...
        <h1 className="hero-title lp-hero-title">
          {config.title.map((line, index) => (
            <span key={index}>
              {index === config.highlightLine ? (
                <span className="hero-title-shine">{line}</span>
              ) : (
                line
              )}
        ...
        <p className="hero-subtitle">
          {config.subtitle.map((line, index) => (
            <span key={index}>
              {line}
              {index < config.subtitle.length - 1 && <br />}
            </span>
          ))}
        </p>
