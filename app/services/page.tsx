...
export default function ServicesPage() {
  useEffect(() => {
    // === Services navigation initialization (previous inline script) ===
    function initServicesPageNavigation() {
      ...
      } as Record<string, { title: string; mobileTitle: string }>;
      ...
      setTimeout(() => {
        document.querySelector('.services-nav')?.classList.add('animate-in');
      }, 100);
      return true;
    }

    // Retry logic in case elements aren't yet in the DOM
    function tryServicesInit(attempt = 0) {
      if (initServicesPageNavigation()) return; // success
      if (attempt < 10) setTimeout(() => tryServicesInit(attempt + 1), 200);
    }

    tryServicesInit(); // initial attempt
  }, []);
  return (
    <>
      <Header />
